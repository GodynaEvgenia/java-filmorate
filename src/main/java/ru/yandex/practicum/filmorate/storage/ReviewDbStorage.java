...
public class ReviewDbStorage {
    ...
    private static final String GET_REVIEW_BASE_QUERY = """
            SELECT review_id, film_id, user_id, useful, is_positive, content
            FROM reviews""";
    ...
    private static final String SAVE_REVIEW_QUERY = """
            INSERT INTO reviews (film_id, user_id, useful, is_positive, content)
            VALUES (?, ?, ?, ?, ?)""";
    private static final String UPDATE_REVIEW_QUERY = """
            UPDATE reviews
            SET useful = ?, is_positive = ?, content = ?
            WHERE review_id = ?""";
    ...
    public Review update(Review filmReview) {
        jdbc.update(UPDATE_REVIEW_QUERY, filmReview.getUseful(), filmReview.getIsPositive(), filmReview.getContent(), filmReview.getReviewId());
    ...
    public Review getById(Integer id) {
        return jdbc.query(GET_REVIEW_BY_ID_QUERY, this::mapRowToReview, id).stream().findAny().orElse(null);
    }
    ...
    public boolean isExists(Integer id) {
        return Boolean.TRUE.equals(jdbc.queryForObject(CHECK_REVIEW_EXISTS_BY_ID_QUERY, (rs, rowNum) -> rs.getBoolean("isExists"), id));
    }
